...
if (!empty($json['auth']) && is_array($json['auth'])) {
  ...
  try {
    ...
    // Register Open Lines connector. Settings/connect UI is in Open Lines → Connectors (PLACEMENT_HANDLER). Do NOT activate any line on install.
    $connectorId = cfg('OPENLINES_CONNECTOR_ID') ?: 'telegram_grey';
    $handlerUrl = $public . '/openlines/handler.php';
    $settingsUrl = $public . '/openlines/settings.php';
    try {
